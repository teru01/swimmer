...
use k8s_openapi::api::autoscaling::v2::{HorizontalPodAutoscaler, HorizontalPodAutoscalerSpec, HorizontalPodAutoscalerStatus};
use k8s_openapi::api::batch::v1::{CronJob, CronJobSpec, CronJobStatus, Job, JobSpec, JobStatus};
use k8s_openapi::api::core::v1::{
    ConfigMap, Container, ContainerStatus, EndpointAddress, EndpointPort, EndpointSubset, Endpoints, Event, EventSource, LimitRange, LimitRangeItem, LimitRangeSpec,
    Namespace, NamespaceSpec, NamespaceStatus, Node, NodeAddress, NodeCondition,
    ...
    ResourceQuota, ResourceQuotaSpec, ResourceQuotaStatus, Secret, Service, ServiceAccount, ServiceSpec, ServiceStatus, VolumeResourceRequirements,
};
...
impl K8sClient for MockK8sClient {
    ...
    async fn get_replicaset(&self, name: &str, namespace: &str) -> Result<ReplicaSet> {
        ...
        Ok(ReplicaSet {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("rs-{}-uid", name), Some(creation_time), Some(labels)),
            spec: Some(ReplicaSetSpec {
                replicas: Some(3),
                selector: LabelSelector {
                    match_labels: Some(selector_labels),
    ...
    async fn get_statefulset(&self, name: &str, namespace: &str) -> Result<StatefulSet> {
        ...
        Ok(StatefulSet {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("sts-{}-uid", name), Some(creation_time), Some(labels)),
            spec: Some(StatefulSetSpec {
                replicas: Some(3),
                service_name: "db-service".to_string(),
                selector: LabelSelector {
    ...
    async fn get_daemonset(&self, name: &str, namespace: &str) -> Result<DaemonSet> {
        ...
        Ok(DaemonSet {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("ds-{}-uid", name), Some(creation_time), Some(labels)),
            spec: Some(k8s_openapi::api::apps::v1::DaemonSetSpec {
                selector: LabelSelector {
                    match_labels: Some(selector_labels),
                    ..Default::default()
    ...
    async fn get_job(&self, name: &str, namespace: &str) -> Result<Job> {
        ...
        Ok(Job {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("job-{}-uid", name), Some(creation_time), None),
            spec: Some(JobSpec {
                completions: Some(1),
                parallelism: Some(1),
                ..Default::default()
    ...
    async fn get_cronjob(&self, name: &str, namespace: &str) -> Result<CronJob> {
        ...
        Ok(CronJob {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("cronjob-{}-uid", name), Some(creation_time), None),
            spec: Some(CronJobSpec {
                schedule: "0 2 * * *".to_string(),
                ..Default::default()
            }),
    ...
    async fn get_configmap(&self, name: &str, namespace: &str) -> Result<ConfigMap> {
        ...
        Ok(ConfigMap {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("cm-{}-uid", name), Some(creation_time), None),
            data: Some(data),
            ..Default::default()
        })
    }
    ...
    async fn get_secret(&self, name: &str, namespace: &str) -> Result<Secret> {
        ...
        Ok(Secret {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("secret-{}-uid", name), Some(creation_time), None),
            type_: Some("Opaque".to_string()),
            data: Some(BTreeMap::new()),
            ..Default::default()
        })
    ...
    async fn get_persistentvolume(&self, name: &str) -> Result<PersistentVolume> {
        ...
        Ok(PersistentVolume {
            metadata: Self::create_metadata(name.to_string(), None, format!("pv-{}-uid", name), Some(creation_time), None),
            spec: Some(PersistentVolumeSpec {
                capacity: Some(capacity),
                access_modes: Some(vec!["ReadWriteOnce".to_string()]),
                persistent_volume_reclaim_policy: Some("Retain".to_string()),
    ...
    async fn get_persistentvolumeclaim(&self, name: &str, namespace: &str) -> Result<PersistentVolumeClaim> {
        ...
        Ok(PersistentVolumeClaim {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("pvc-{}-uid", name), Some(creation_time), None),
            spec: Some(PersistentVolumeClaimSpec {
                access_modes: Some(vec!["ReadWriteOnce".to_string()]),
                resources: Some(VolumeResourceRequirements {
                    requests: Some(requests),
    ...
    async fn get_storageclass(&self, name: &str) -> Result<StorageClass> {
        ...
        Ok(StorageClass {
            metadata: Self::create_metadata(name.to_string(), None, format!("sc-{}-uid", name), Some(creation_time), None),
            provisioner: "kubernetes.io/aws-ebs".to_string(),
            parameters: Some(parameters),
            ..Default::default()
        })
    ...
    async fn get_serviceaccount(&self, name: &str, namespace: &str) -> Result<ServiceAccount> {
        ...
        Ok(ServiceAccount {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("sa-{}-uid", name), Some(creation_time), None),
            automount_service_account_token: None,
            image_pull_secrets: None,
            secrets: None,
        })
    ...
    async fn list_endpoints(&self, _namespace: Option<&str>) -> Result<Vec<Endpoints>> {
        ...
        Ok(vec![
            Endpoints {
                metadata: Self::create_metadata("web-service".to_string(), Some("default".to_string()), "ep-1".to_string(), Some(creation_time), None),
                subsets: Some(vec![
                    EndpointSubset {
                        addresses: Some(vec![
                            EndpointAddress {
    ...
    async fn get_endpoints(&self, name: &str, namespace: &str) -> Result<Endpoints> {
        ...
        Ok(Endpoints {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("ep-{}-uid", name), Some(creation_time), None),
            subsets: Some(vec![
                EndpointSubset {
                    addresses: Some(vec![
                        EndpointAddress {
    ...
    async fn list_events(&self, _namespace: Option<&str>) -> Result<Vec<Event>> {
        let creation_time = Time(chrono::DateTime::parse_from_rfc3339("2024-01-15T10:00:00Z").unwrap().with_timezone(&chrono::Utc));
        let first_timestamp = Time(chrono::DateTime::parse_from_rfc3339("2024-01-15T10:00:00Z").unwrap().with_timezone(&chrono::Utc));
        let last_timestamp = Time(chrono::DateTime::parse_from_rfc3339("2024-01-15T10:05:00Z").unwrap().with_timezone(&chrono::Utc));

        Ok(vec![
            Event {
                metadata: Self::create_metadata("web-app-1.1234567890".to_string(), Some("default".to_string()), "evt-1".to_string(), Some(creation_time), None),
    ...
    async fn get_event(&self, name: &str, namespace: &str) -> Result<Event> {
        ...
        Ok(Event {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("evt-{}-uid", name), Some(creation_time), None),
            action: Some("Started".to_string()),
            count: Some(1),
            event_time: None,
            first_timestamp: Some(first_timestamp),
    ...
    async fn list_horizontalpodautoscalers(&self, _namespace: Option<&str>) -> Result<Vec<HorizontalPodAutoscaler>> {
        let creation_time = Time(chrono::DateTime::parse_from_rfc3339("2024-01-15T10:00:00Z").unwrap().with_timezone(&chrono::Utc));

        Ok(vec![
            HorizontalPodAutoscaler {
                metadata: Self::create_metadata("web-hpa".to_string(), Some("default".to_string()), "hpa-1".to_string(), Some(creation_time), None),
    ...
    async fn get_horizontalpodautoscaler(&self, name: &str, namespace: &str) -> Result<HorizontalPodAutoscaler> {
        ...
        Ok(HorizontalPodAutoscaler {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("hpa-{}-uid", name), Some(creation_time), None),
            spec: Some(HorizontalPodAutoscalerSpec {
                scale_target_ref: Default::default(),
                min_replicas: Some(2),
                max_replicas: 10,
    ...
    async fn list_limitranges(&self, _namespace: Option<&str>) -> Result<Vec<LimitRange>> {
        ...
        Ok(vec![
            LimitRange {
                metadata: Self::create_metadata("default-limits".to_string(), Some("default".to_string()), "lr-1".to_string(), Some(creation_time), None),
    ...
    async fn get_limitrange(&self, name: &str, namespace: &str) -> Result<LimitRange> {
        ...
        Ok(LimitRange {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("lr-{}-uid", name), Some(creation_time), None),
            spec: Some(LimitRangeSpec {
                limits: vec![
                    LimitRangeItem {
                        type_: "Container".to_string(),
    ...
    async fn list_resourcequotas(&self, _namespace: Option<&str>) -> Result<Vec<ResourceQuota>> {
        ...
        Ok(vec![
            ResourceQuota {
                metadata: Self::create_metadata("default-quota".to_string(), Some("default".to_string()), "rq-1".to_string(), Some(creation_time), None),
    ...
    async fn get_resourcequota(&self, name: &str, namespace: &str) -> Result<ResourceQuota> {
        ...
        Ok(ResourceQuota {
            metadata: Self::create_metadata(name.to_string(), Some(namespace.to_string()), format!("rq-{}-uid", name), Some(creation_time), None),
            spec: Some(ResourceQuotaSpec {
                hard: Some(hard.clone()),
                scope_selector: None,
                scopes: None,
